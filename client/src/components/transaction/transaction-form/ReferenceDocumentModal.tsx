...
const ReferenceDocumentModal = ({transactionWatch, transactionSetValue,document,documentIndex,trigger }: ReferenceDocumentModalProps) => {
    ...
    return(
        <Dialog>
            ...
            <DialogContent className="min-w-[90%] max-h-[90%] overflow-auto">
                <DialogHeader>
                    <DialogTitle className="capitalize">upload reference documents</DialogTitle>
                    <DialogDescription>
                ...
                <DialogFooter>
                    <EpicButton label="save" type="button" className="w-[100px] uppercase" onClick={handlePopupSubmit((data) => {
                        const currentDocs = transactionWatch("upload_reference_documents") || [];
                        if(documentIndex !== -1) {
                            // Update existing document
                            const updatedDocs = [...currentDocs];
                            updatedDocs[documentIndex] = data;
                            transactionSetValue("upload_reference_documents", updatedDocs);
                        } else {
                            // Add new document
                            transactionSetValue("upload_reference_documents", [...currentDocs, data]);
                        }
                        resetPopupForm();
